def generator_lambda_link(link):
    ...
    result_link = [f"""
{generator_header_lambda_link(first_entity, second_entity, partition_key, sort_key)}
"""]
    for api in link['API']:
        if api['type'] == 'PUT':
            result_link.append(generator_case_put_link(api['name'], partition_key, sort_key))
        elif api['type'] == 'DELETE':
            result_link.append(generator_case_delete_link(api['name'], partition_key, sort_key))
        elif api['type'] == 'GET':
            result_link.append(generator_case_get_link(api['name'], partition_key, sort_key))
        elif api['type'] == 'POST':
            result_link.append(generator_case_update_link(api['name'], partition_key, sort_key, api['parameters']))
    return ''.join(result_link)


def generator_header_lambda_link(first_entity, second_entity, partition_key, sort_key):
    return f"""
...
def generator_case_put_link(api_name, partition_key, sort_key):
    return f"""
        case '{api_name}':
            response = dynamodb_manager.create_link({partition_key}, {sort_key}, event['arguments'])
            if response['ResponseMetadata']['HTTPStatusCode'] == 200:
                response = f"Link created"
    """


def generator_case_delete_link(api_name, partition_key, sort_key):
    return f"""
        case '{api_name}':
            response = dynamodb_manager.delete_item({partition_key}, {sort_key})
...
def generator_case_get_link(api_name, partition_key, sort_key):
    return f"""
        case '{api_name}':
